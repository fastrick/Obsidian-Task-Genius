...
describe('OnCompletionManager', () => {
	...
	describe('Configuration Parsing', () => {
		describe('Simple Format Parsing', () => {
			...
			it('should handle case-insensitive parsing', () => {
				...
				expect(result1.config?.type).toBe(OnCompletionActionType.DELETE);
				expect(result2.isValid).toBe(true);
				expect(result2.config?.type).toBe(OnCompletionActionType.KEEP);
				expect(result3.isValid).toBe(true);
				expect(result3.config?.type).toBe(OnCompletionActionType.ARCHIVE);
			});
	...
	describe('Configuration Validation', () => {
		it('should validate delete configuration', () => {
			const config: OnCompletionConfig = { type: OnCompletionActionType.DELETE };
			expect(manager['validateConfig'](config)).toBe(true);
		});

		it('should validate keep configuration', () => {
			const config: OnCompletionConfig = { type: OnCompletionActionType.KEEP };
			expect(manager['validateConfig'](config)).toBe(true);
		});
		...
		it('should validate archive configuration', () => {
			const config: OnCompletionConfig = { type: OnCompletionActionType.ARCHIVE };
			expect(manager['validateConfig'](config)).toBe(true);
		});

		it('should validate duplicate configuration', () => {
			const config: OnCompletionConfig = { type: OnCompletionActionType.DUPLICATE };
			expect(manager['validateConfig'](config)).toBe(true);
		});
	...
	describe('Task Completion Event Handling', () => {
		...
		it('should handle task completion with valid onCompletion config', async () => {
			...
			expect(manager.executeOnCompletion).toHaveBeenCalledWith(
				mockTask,
				{ type: OnCompletionActionType.DELETE }
			);
		});
		...
		it('should handle execution errors gracefully', async () => {
			manager.executeOnCompletion = jest.fn().mockRejectedValue(new Error('Execution error'));
