...
const translations = {
  ...
  "Configure how tasks inherit metadata from file frontmatter": "タスクがファイルフロントマターからメタデータを継承する方法を設定",
  "Enable file metadata inheritance": "ファイルメタデータ継承を有効化",
  "Allow tasks to inherit metadata properties from their file's frontmatter": "タスクがそのファイルのフロントマターからメタデータプロパティを継承することを許可",
  "Inherit from frontmatter": "Inherit from frontmatter",
  "Tasks inherit metadata properties like priority, context, etc. from file frontmatter when not explicitly set on the task": "タスクに明示的に設定されていない場合、タスクは優先度、コンテキストなどのメタデータプロパティをファイルフロントマターから継承",
  "Inherit from frontmatter for subtasks": "Inherit from frontmatter for subtasks",
  "Allow subtasks to inherit metadata from file frontmatter. When disabled, only top-level tasks inherit file metadata": "サブタスクがファイルフロントマターからメタデータを継承することを許可。無効化した場合、トップレベルのタスクのみがファイルメタデータを継承",
